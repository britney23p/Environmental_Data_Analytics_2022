...
#summary-- the new object that was created was not a data frame -- car_speed only contained Values and was not a data frame.
```
...
#summary-- assigning a name and using as.data.frame() means that you are creating a data frame-- df_car_speed is no longer just values from a file.
```
...
summary(df) #shows the summary of dist and speed that you would see in boxplot
...
df <- df[,-2]
df
#summary--this deleted the second column, which was car_dist
```
