...
> Answer: 
This figure suggests that there is a negative correlation between depth and temperature. The distribution of points suggest 
...
> Answer: 
-The R-squared value is 0.7387. This means that 73.8% variability in temperature is explained by changes in depth.
-The degrees of freedom on which the findings are based is 9726.
-The p-value was is than 0.05, so the results were statistically significant. This means that we would reject the null hypothesis and say that there is a difference in the mean lake temperature with depth across all lakes during the month of July.
-The temperature is predicted to decrease by 1.95 degrees Celcius for every 1m change in depth.
...
HSD.temp <- HSD.test(temp_totals_anova, "lakename", TRUE)
print(HSD.temp)
...
>Answer:
Based on the Tukey's HSD test, the lake that has the same mean temperature as Peter Lake is Paul Lake. 
