...
Rainfall_BR <- read.csv("/Users/britneypepper/Desktop/ENVIRON 872 and L/GitHubRepositories/Environmental_Data_Analytics_2022/Data/Raw/pr_1901_2016_BRA.csv", stringsAsFactors = TRUE)
...
BRA_rain_ts <- ts(BRA_complete$Rainfall, start = c(fmonth, fyear), frequency = 12)
BRA_temp_ts <- ts(BRA_complete$Temperature_C, start = c(fmonth, fyear), frequency = 12)
...
BR_rain_decomposed <- stl(BRA_rain_ts, s.window = "periodic")
plot(BR_rain_decomposed)

BR_temp_decomposed <- stl(BRA_temp_ts, s.window = "periodic")
...
BR_Rain_Componenets <-
  as.data.frame(BR_rain_decomposed$time.series[,1:3])
BR_Temp_Componenets <- 
  as.data.frame(BR_temp_decomposed$time.series[,1:3])

BR_Rain_Componenets <- mutate(BR_Rain_Componenets, Observed = BRA_complete$Rainfall, Date = BRA_complete$Date)
BR_Temp_Componenets <- mutate(BR_Temp_Componenets, Observed = BRA_complete$Rainfall, Date = BRA_complete$Date)

#Using these componenets to make ggplot later
