...
neuse <- st_read('./Data/Spatial/NCHHUC8.shp') %>%
  neuse <- st_read() %>%
  filter(SUBBASIN == 'Upper Neuse')

##watch video for rest
...
HUC8_wind_energy <- read.csv("https://raw.githubusercontent.com/ENV859/EnviroAtlasData/main/Wind_Energy.csv", colClasses = c('HUC_12' = 'factor')) %>% 
  mutate(HUC_8 = subtr(HUC_12, 0, 8)) %>%
  group_by(HUC) %>%
...
clipped_HUCs <- huc8_sf_utm %>%
  st_intersectaion(triangle_sf)

mapview(clipped_HCSs)
