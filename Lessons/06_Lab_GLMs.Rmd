...
PeterPaul.chem.nutrients <- read.csv("/Users/britneypepper/Desktop/ENVIRON 872 and L/GitHubRepositories/Environmental_Data_Analytics_2022/Data/Processed/NTL-LTER_Lake_Chemistry_Nutrients_PeterPaul_Processed.csv", stringsAsFactors = TRUE)
# Set date to date format
PeterPaul.chem.nutrients$sampledate <- as.Date(PeterPaul.chem.nutrients$sampledate, format = "%Y-%m-%d")

EPAair <- read.csv("/Users/britneypepper/Desktop/ENVIRON 872 and L/GitHubRepositories/Environmental_Data_Analytics_2022/Data/Processed/EPAair_O3_PM25_NC1819_Processed.csv", stringsAsFactors = TRUE)
# Set date to date format
EPAair$Date <- as.Date(EPAair$Date, format = "%Y-%m-%d")


Litter <- read.csv("/Users/britneypepper/Desktop/ENVIRON 872 and L/GitHubRepositories/Environmental_Data_Analytics_2022/Data/Processed/NEON_NIWO_Litter_mass_trap_Processed.csv", stringsAsFactors = TRUE)
# Set date to date format
...
### Formulating Hypothesis for µ (mean of the series)
...
> Answer: All ozone levels have a value of 50 or below  for the AQI index.
...
#1.
ozonedensity <- ggplot(EPAair, aes(x = Ozone, fill = as.factor(Year))) + 
  geom_density(alpha = 0.5)+
  scale_fill_mutual(values = c("darkgreen", "blue")) + 
  labs(x = "Ozone AQI value:") +
  geom_density()
summary(ozonedensity)
#???? watch video


o3.boxplot <-
  ggplot(EPAair, aes(x = as.factor(Year), u = Oxone)) +
  geom_boxplot() +
  stat_summary(geom =  Z())
...
o3.violin <-
  ggplot(EPAair, ares(x = as.factor(Year), y  = Ozone)) +
  geom_violin(drQ))
...
linereg <- ggplot(EPAair, aes(x = Ozone, fill = as.factor(Year)))
...
OzonebyALL.regression <- lm(data = EPAair.subset, MP2.5 ~ Ozone + Year + Month + meanLng + meanLat)
step(OxonebyALL.regression)
...
OzoneBest.regression <- lm(data = EPAair.subset, PM2.5 ~ Ozone + Month +
                             meanLng + meanLat)
